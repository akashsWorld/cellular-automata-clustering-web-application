...
const findBestResult = async ( data ,clusterNumber)=>{
    ...
    let a= 100;
    while(a--){
            const levelOneClustersResponse  = await fetch(`http://localhost:9788/cluster/FindCluster/levelOne?groups=2,2,2&neighbourHood=3&boundary=nullBoundary`,{
                method:'POST',
                headers:{
        ...
        if(levelOneClustersResponse.status === 200){
            ...
            const levelTwoResponse = await fetch(`http://localhost:9788/cluster/FindCluster/levelTwo?requiredCluster=${clusterNumber}&neighbourHood=3&boundary=nullBoundary`,{
                method:'POST',
                headers:{
                    'Content-Type':'application/json'
                },
            ...
            if(levelTwoResponse.status === 200){
                ...
                const validationResponse = await fetch(`http://localhost:8000/valid/get`,{
                    method:'POST',
                    headers:{
                        'Content-Type':'application/json'
                    },
